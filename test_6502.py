...
try:
    for i in range(200):
        ...
        print(f"{cpu.current_instruction_pc:04X} | {inst_str:<8} | {cpu.registers.A:02X} {cpu.registers.X:02X} {cpu.registers.Y:02X} {cpu.registers.stack_pointer:02X} |", cpu.registers.register_output())
    ...
    print(f"SP: 0x{cpu.registers.stack_pointer:02X}")
    print()
    ...
    for i in range(0x100):
        if not (i % 16):
            print(f"{i + 0x100:04X} | ", end='', flush=True)
        b = bus.read(i + 0x100)
        print(f'{b:02X}', end=' ', flush=True)
        if not (i + 1) % 16:
            print()
